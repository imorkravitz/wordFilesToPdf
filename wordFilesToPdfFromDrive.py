...
def download_and_convert_files(service, source_folder_id, download_path, destination_path):
    ...
    for item in items:
        ...
        try:
            download_and_convert_file(service, file_id, file_name, download_path, destination_path)
...
def download_and_convert_file(service, file_id, file_name, download_path, destination_path):
...
def upload_files(service, upload_path, folder_id):
    files_to_upload = [f for f in os.listdir(upload_path) if
                       os.path.isfile(os.path.join(upload_path, f)) and not f.startswith('.') and f.lower().endswith(
                           '.pdf') and '.DS_Store' not in f and "subset" not in f]
    ...
    if len(files_to_upload) > 0:
        for pdf_file in files_to_upload:
            ...
            if os.path.getsize(pdf_file_path) < 1 * 512 * 512:  # Check if file size is less than 1 MB
                temp_delete_list.append(pdf_file_path)
                continue
    ...
    # Process temp_delete_list to remove entries from copiedFileLog.txt
    if temp_delete_list:
        with open(LOG_FILE_PATH, 'r') as log_file:
            copied_files = log_file.readlines()
        copied_files = [line.strip() for line in copied_files]

        # Remove entries from copied_files
        updated_copied_files = [file for file in copied_files if file not in temp_delete_list]

        # Write updated list back to the log file
        with open(LOG_FILE_PATH, 'w') as log_file:
            for file in updated_copied_files:
                log_file.write(file + '\n')

        # Log deleted files
        for file in temp_delete_list:
